		...
		el.addEventListener(data.on, function () {
			...
			setTimeout(function () {
				...
				if(data.src == '#north') {
					...
					//specific to this scene photo from north
					skybox.setAttribute('rotation', {x:0, y: 60, z: 0});
				...
				if(data.src == '#south') {
					...
					west.setAttribute('material', {src: '#thumbW'});
					answer.setAttribute('material', {src: '#thumbanswer'});
					//specific to this scene photo from south
					...
					//you've changed it.
					skybox.setAttribute('rotation', {x:0, y: 63, z: 0});
					for (var i = 0; i < keylist.length; i++) {
						keylist[i].setAttribute('class', '');
						keylist[i].setAttribute('visible', false);
					}
				...
				if(data.src == '#east') {
					...
					west.setAttribute('material', {src: '#thumbW'});
					answer.setAttribute('material', {src: '#thumbanswer'});
					//specific to this scene photo from east
					skybox.setAttribute('rotation', {x:0, y: 63, z: 0});
					for (var i = 0; i < keylist.length; i++) {
						keylist[i].setAttribute('class', '');
						keylist[i].setAttribute('visible', false);
				...
				if(data.src == '#west') {
					...
					answer.setAttribute('material', {src: '#thumbanswer'});
					//specific to this scene photo from west
					skybox.setAttribute('rotation', {x:0, y: 63, z: 0});
					for (var i = 0; i < keylist.length; i++) {
						keylist[i].setAttribute('class', '');
						keylist[i].setAttribute('visible', false);
				...
				if(data.src == '#south' && data.iskey == true) {
					...
					//specific to this scene photo from south for answer key
